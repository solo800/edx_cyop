...
The best-performing model, XGBoost, achieved an R² of 0.679 on the held-out 2024 test set, explaining roughly two-thirds of house price variance using nine features. The single most impactful improvement came not from algorithm selection but from feature engineering when we introduced commune-level median income data. It boosted tree model R² by approximately 8 points, nearly doubling the gap over linear regression. This finding reinforces a key lesson from the course that the quality of your features matters more than the sophistication of your algorithm.
...
Random Forest handles non-linear relationships and feature interactions naturally — no need to specify them manually. The 500-tree ensemble learns city-specific patterns through recursive splits. This is the key advantage over linear regression: the model can discover that, say, land area matters more in Toulouse than in Paris without being told.
...
XGBoost (extreme gradient boosting) builds an ensemble of shallow decision trees sequentially, with each tree correcting the errors of the previous ones. It requires numeric input, so I one-hot encoded the `target_city` factor. I used a 20% validation holdout from the training data for early stopping — this prevents the test set from influencing model selection (number of boosting rounds), which would bias the test metrics upward.

```{r xgb-model, eval=FALSE}
set.seed(42)
...
train_x <- make_matrix(train_model_xgb)
val_x <- make_matrix(val_model_xgb)
test_x <- make_matrix(test_model)

dtrain <- xgb.DMatrix(data = train_x, label = train_model_xgb$price)
dval <- xgb.DMatrix(data = val_x, label = val_model_xgb$price)
dtest <- xgb.DMatrix(data = test_x, label = test_model$price)
...
xgb_results <- evaluate_model(
    test_model$price, xgb_pred_test,
    as.character(test_model$target_city), "XGBoost"
)
...
XGBoost achieved the best performance with an R² of `r sprintf("%.3f", overall$r2[overall$model == "XGBoost"])`, closely followed by Random Forest (`r sprintf("%.3f", overall$r2[overall$model == "Random Forest"])`). Both tree models substantially outperformed linear regression, reducing RMSE by approximately 11% and MAE by 17%.

The narrow gap between Random Forest and XGBoost (R² difference of just 0.003) suggests that the performance ceiling is driven by *missing features* rather than model complexity. The DVF dataset does not include property condition, renovation status, energy performance rating, proximity to transit, or school quality — all of which influence house prices. A more sophisticated algorithm cannot recover information that is not in the data.
...
The per-city results reveal important heterogeneity. **Paris is the most predictable market** (R² ≈ 0.74), likely because the large income variation across its many communes provides a strong price signal. **Toulouse is the hardest to predict** (R² ≈ 0.43 for XGBoost) despite having the most training data — its market is simply more heterogeneous, with price variation driven by factors our features cannot capture.

Notably, Annecy (the smallest sample with only 419 test transactions) performs mid-pack (R² ≈ 0.57), suggesting that sample imbalance is not the primary driver of per-city performance differences.
...
Both models agree that **surface area** is the dominant predictor, which is unsurprising — bigger houses cost more. The more interesting finding is that **commune-level median income** ranks as the second most important feature in XGBoost (19.2% of total gain). This confirms the EDA insight: neighborhood wealth is a powerful proxy for desirability, school quality, and amenities. The city identifier and ring (center vs. suburb) provide geographic context, while rooms, land area, and temporal features round out the picture.
...
Tree models gained roughly 8 R² points — nearly doubling the improvement gap over linear regression. The reason is that tree models can use income to create non-linear interaction effects (e.g., "if income > €25,000 AND surface > 100 m², predict a premium"), while linear regression only gets a single additive coefficient.
...
The PCA biplot shows that clusters cut across city boundaries — Paris communes appear in multiple tiers, and some suburban communes around Lyon and Annecy share market characteristics with central Marseille. This cross-city clustering adds nuance to the relocation decision: it is not just about choosing a city, but about choosing the right neighborhood tier within that city.
...
The final ranking combines three dimensions with explicit weights: lifestyle (50%, from the composite screening score), affordability (30%, inverted predicted price), and model predictability (20%, XGBoost R²). Including predictability rewards cities where our model's predictions are more reliable — useful when the prediction will inform an actual purchase decision.
...
**Toulouse emerges as the top-ranked city**, with the best balance of lifestyle quality, housing affordability, and a reasonably predictable market. It offers roughly 2,000 hours of annual sunshine, a low far-right vote share, and the most affordable standard house price among the candidates. Marseille and Lyon follow as strong alternatives — Marseille offers the most sunshine but a somewhat less predictable market, while Lyon provides excellent infrastructure but at a higher price point.

Paris, despite having the highest model predictability (R² = 0.74), ranks last due to its extreme prices and lower lifestyle scores. This confirms the intuition that motivated the project: Paris is a great city to leave, not to buy a house in.
...
The key technical findings are: (1) XGBoost and Random Forest substantially outperform linear regression for house price prediction, explaining about 68% of price variance on the 2024 test set; (2) commune-level median income is a remarkably powerful feature, boosting tree model R² by approximately 8 points; and (3) model performance varies significantly across cities, with Paris being most predictable and Toulouse most challenging, in ways that are not explained by sample size alone.

The practical conclusion is that Toulouse offers the best combination of affordable housing, favorable climate, and community alignment for my specific priorities. The framework itself — weighted composite scoring plus ML-based affordability analysis — is reusable for anyone with different preferences.

## Limitations

Several limitations affect the analysis:

The R² of 0.68 means roughly one-third of price variance remains unexplained. The DVF dataset lacks critical property attributes: house condition, renovation status, energy performance rating (DPE), proximity to public transit, and school catchment quality. These omissions likely explain much of the residual error and the heterogeneity in per-city performance.

The commune-level income data is from a single year (Filosofi 2020) and is applied uniformly to transactions from 2020 through 2024. In reality, neighborhood incomes evolve over time. The 2020-only data for the first year covers only the second semester, introducing a mild seasonal imbalance.
...
Finally, the temporal train/test split, while realistic, means the model is trained predominantly on a period of rising prices (2020–2022) and cooling prices (2023), then tested on a relatively stable 2024. A future market shift could degrade performance.

## Future Work

Several extensions could improve the analysis. Incorporating DPE (energy performance) ratings, which are increasingly available via open data, would add a significant predictor. Geospatial features — distance to nearest metro station, school quality index, proximity to green space — could be computed from OpenStreetMap data and would likely close much of the remaining R² gap.
...
For the relocation decision itself, a natural next step is to use the trained model as a property valuation tool — feeding in specific listings to identify undervalued houses in the target city.
